<template>
  <table v-if="this.totales.expedientes" id="customers">
    <tr>
    ...
    <tr
      ...
      :key="index"
      @click="this.$router.push(`/expedientes/${expediente._id}`)"
    >
      <td>{{ expediente.nombre }}</td>
      <td>{{ formatDate(expediente.fechaActual) }}</td>
      <!-- <td>{{ formatDate2(expediente.fechaNacimiento) }}</td> -->
      <td>{{ expediente.edad }} Años</td>
      <td>{{ expediente.sexo }}</td>
      <td :class="hasDxColor(expediente.diagnosticoPresuntivo)">
        <i :class="hasDxIcon(expediente.diagnosticoPresuntivo)"></i>
      </td>
    </tr>
    <tr>
      <td>Total: {{ formatNumber(this.totales.expedientes) }}</td>
      <td></td>
      <!-- <td></td> -->
      <td></td>
      <td></td>
  ...
  <h6 class="noRegist" v-if="!this.totales.expedientes">
...
import { updateOne } from "@/services/expedientesclinicos/Expediente";
...
export default {
  ...
  methods: {
    ...
    async marcarListo(expediente: Expediente) {
      alert("Expediente Lista.");
      try {
        const res = await updateOne(expediente);
      } catch (error) {
        // console.error(error);
      }
    },
    ...
    valorTotal() {
      this.totales.expedientes = this.expedientes.length;
      this.totales.valor = this.expedientes.reduce(
