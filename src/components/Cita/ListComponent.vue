<template>
  <table v-if="this.totales.citas" id="customers">
    <tr>
    ...
    <tr
      ...
      :key="index"
      @click="this.$router.push(`/citas/${cita._id}`)"
    >
      <td>{{ cita.seguro }}</td>
      <td>{{ formatDate(cita.fechaActual) }}</td>
      <!-- <td>{{ formatDate2(cita.fechaNacimiento) }}</td>
      <td>{{ cita.edad }} Años</td>
      <td>{{ cita.sexo }}</td> -->
    </tr>
    <tr>
      <td>Total: {{ formatNumber(this.totales.citas) }}</td>
      <td></td>
      <!-- <td></td>
      <td></td>
      <td class="der"></td> -->
  ...
  <h6 class="noRegist" v-if="!this.totales.citas">
...
import { updateOne } from "@/services/expedientesclinicos/Cita";
...
export default {
  ...
  methods: {
    ...
    async marcarListo(cita: Cita) {
      alert("Cita Lista.");
      try {
        const res = await updateOne(cita);
      } catch (error) {
        // console.error(error);
      }
    },
    ...
    valorTotal() {
      this.totales.citas = this.citas.length;
      this.totales.valor = this.citas.reduce(
