<template>
  ...
  <body v-show="!cargando">
    <div class="grid-layout">
      ...
      <div class="caja c1">
        <h4 class="bold">{{ this.cxp.medico }}</h4>
        <h5>{{ this.cxp.direccion }}</h5>
        <!-- <h5>
          <span class="bold">RNC / Cédula: </span>
          <span>{{ this.cxp.rncMedico }}</span>
        </h5> -->
        <h5>
      ...
      <div class="caja c2">
        <h3 class="bold">Cxp<br /></h3>
        <h5>
          <span class="bold">No: </span>
          <span>{{ formatSecuence(cxp.no) }}</span>
        </h5>
        <!-- <h5>
        ...
        <h5>
          ...
          <span>{{ formatDate(this.cxp.fechaProceso) }}</span>
        </h5>
        <!-- <h5>
          <span class="bold">Vencimiento: </span>
          <span>{{ formatDate(this.cxp.vencimiento) }}</span>
        </h5> -->
      </div>
      ...
      <div class="caja c3">
        <!-- <h5>
          ...
          <span>{{ this.cxp.rncCliente }}</span>
        </h5> -->
        <h5>
          <span class="bold">Cliente: </span>
          <span>{{ this.cxp.descCliente }}</span>
        </h5>
      </div>
      <div class="caja c4">
        <table class="table table-hover">
          ...
          <tr>
            <td class="t izq">{{ this.cxp.descServicios }}</td>
            <td class="t der">
              {{ formatNumber(this.cxp.precxp) }}
            </td>
          </tr>
        </table>
      </div>
      ...
      <div class="caja c6">
        <h6>{{ formatNumber(this.cxp.precxp) }}</h6>
        <h6>{{ formatNumber(this.cxp.descuento) }}</h6>
        <h6>{{ formatNumber(this.cxp.descuento) }}</h6>
        <h6 class="bold">
          {{ formatNumber(this.cxp.precxp) }}
        </h6>
...
export default defineComponent({
  ...
  methods: {
    ...
    async loadCxp(id: string) {
      ...
      if (this.$store.state.ids.length == 0) {
        this.$router.push(`/cxps/${this.cxp._id}`);
      } else {
        this.$router.push(`/cxpslot/new`);
      }
    ...
    async handleDelete() {
      try {
        if (typeof this.$route.params.id === "string") {
          deleteCxp(this.$route.params.id);
          this.$router.push("/cxps");
        }
      } catch (error) {
        //console.error(error);
  ...
  mounted() {
    if (typeof this.$route.params.id === "string") {
      this.loadCxp(this.$route.params.id);
    }
  },
