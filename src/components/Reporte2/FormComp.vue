    ...
    <form>
      <fieldset>
        ...
        <div class="d-grid gap-2">
          <button
            :disabled="
              !reporte.description || !reporte.type || !reporte.solicitado
            "
...
import { getReportes } from "@/services/expedientesclinicos/ReporteService";
...
export default defineComponent({
  ...
  methods: {
    ...
    async saveReportea() {
      ...
      try {
        const res = await createReportea(this.servicio);
        // // console.log(res);
      } catch (error) {
        // // console.error(error);
      }
