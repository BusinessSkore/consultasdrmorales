      ...
      <div v-show="!cargando" class="general">
          ...
          <div v-if="showModal" class="modal">
            <div class="contenedor">
              ...
              <div class="contenido">
                ...
                <div class="contenido">
                  <Filters
                    :filterLigas="filterLigas"
                    :search="search"
                    :filteredLigas="filteredLigas"
                  />
                </div>
              </div>
            </div>
        ...
        > -->
            <h4>
              ...
              <i
                style="cursor:pointer"
                @click="this.$router.push(`/ligas/new`)"
                class="fas fa-plus"
                _mstvisible="2"
              ></i>
            </h4>
            <ListadoComponent :ligas="ligas" v-show="!cargando" />
          </div>
        </div>
      </div>
    </Transition>
...
<script lang="ts">
import Filters from "@/components/Liga/FiltersComponent.vue";
import ListadoComponent from "@/components/Liga/ListComponent.vue";
...
export default {
  ...
  methods: {
    ...
    async filterLigas(catName: string) {
      try {
      ...
      if (catName !== "Todos") {
        this.ligas = this.ligas.filter((liga: Liga) => {
          return liga.descripcion === catName;
        });
      }
    },
    ...
    async search(term: string) {
      ...
      if (term !== "Todos") {
        this.ligas = this.ligas.filter((liga: Liga) => {
          return liga.descripcion.toLowerCase().includes(term.toLowerCase());
        });
      }
      this.toggleLoading();
    },
...
.grid {
  ...
  grid-template-areas:
    ...
    "ligas ligas ligas ligas ligas ligas ligas";
}
