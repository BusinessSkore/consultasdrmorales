<template>
  <table v-if="this.totales.ligas" id="customers">
    <tr>
      <th>Descripcion</th>
    </tr>
    <tr
      ...
      :key="index"
      @click="this.$router.push(`/ligas/${liga._id}`)"
    >
      <td>{{ liga.descripcion }}</td>
    </tr>
    <tr>
      <td>Total: {{ formatNumber(this.totales.ligas) }}</td>
    </tr>
  </table>

  <h6 class="noRegist" v-if="!this.totales.ligas">
...
import { updateOne } from "@/services/expedientesclinicos/Liga";
...
export default {
  ...
  methods: {
    ...
    async marcarListo(liga: Liga) {
      alert("Liga Lista.");
      try {
        const res = await updateOne(liga);
      } catch (error) {
        // console.error(error);
      }
    },
    ...
    valorTotal() {
      this.totales.ligas = this.ligas.length;
      this.totales.cobertura = this.ligas.reduce(
