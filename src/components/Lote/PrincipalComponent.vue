<template v-show="!cargando">
  <!-- <p>Original: {{this.lotesFiltradas}}</p><br/><br/> -->
  <!-- <p>Filtrado: {{this.lotes}}</p><br/><br/> -->
          ...
          <div>
            ...
            <nav :class="navMenuClas2">
              <a
                class="navbar-option"
                href="#"
                @click="this.$router.push('/lotes/new')"
                ><i class="fas fa-file"></i> Registrar Lote</a
              >
              <!-- <a
                class="navbar-option"
                href="#"
                @click="this.$router.push('/lotesciru/new')"
...
<script lang="ts">
import Filters from "@/components/Lote/FiltersComponent.vue";
import ListadoComponent from "@/components/Lote/ListComponent.vue";
...
export default {
  ...
  methods: {
    ...
    filtrarEsp() {
      this.lotes = this.lotesFiltradas.filter((lote: Lote) => {
        return lote.nombPac
          .toLowerCase()
    ...
    async filterLotes(catName: string) {
      try {
        const res = await getLotes(this.criterio);
        this.lotes = res.data;
        this.lotesFiltradas = res.data;
      } catch (error) {
        // console.error(error);
      }
      if (catName !== "Todos") {
        this.lotes = this.lotes.filter((lote: Lote) => {
          return lote.aflNSS === catName;
        });
      }
    },
    ...
    async search(term: string) {
      ...
      if (term !== "Todos") {
        this.lotes = this.lotes.filter((lote: Lote) => {
          return lote.aflNSS.toLowerCase().includes(term.toLowerCase());
        });
      }
      this.toggleLoading();
    },
...
.grid {
  ...
  grid-template-areas:
    ...
    "lotes lotes lotes lotes lotes lotes lotes";
}
