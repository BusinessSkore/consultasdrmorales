        ...
        <form>
          <fieldset>
            <div class="card ">
              <h6>{{ encabezado }}</h6>
              <div class="card-body">
                <label class="form-label"><b>Datos del Lote</b></label>
                <div class="grid-6">
                  <!-- <div>
                    <label
            ...
            <div class="card" v-if="this.reclamos.cantTotal > 0">
              <div class="card-body">
                <label class="form-label"
                  ><b>Detalle de Reclamaciones</b></label
                >
            ...
            <button
              ...
              :disabled="
                !lote.fechAutor || !lote.nombARS || !this.reclamos.cantTotal > 0
              "
            >
...
export default defineComponent({
  ...
  async mounted() {
    ...
    if (this.$route.fullPath == "/lotes/new") {
      ...
      this.modoForm = "show";
      this.encabezado = "Detalles de Lote de Reclamaciones";
    }
  ...
  methods: {
    ...
    async loadReclamosLote(seguro: string) {
      this.toggleLoading();
      try {
        const { data } = await getReclamosLote(seguro);
        this.reclamos = data;
        // this.fixTime();
      } catch (error) {
        //console.error(error);
    ...
    // async putReclamosLote(seguro: string, loteNo: number) {
    //   this.toggleLoading();
    //   try {
    //     const { data } = await postReclamosLote(seguro, loteNo);
    //     this.reclamos = data;
    //     // this.fixTime();
    //   } catch (error) {
    //     //console.error(error);
