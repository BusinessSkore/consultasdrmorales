<template>
  ...
  <body v-show="!cargando">
    <div class="grid-layout">
      ...
      <div class="caja c1">
        <h4 class="bold">{{ this.factura.medico }}</h4>
        <h5>{{ this.factura.direccion }}</h5>
        <!-- <h5>
          <span class="bold">RNC / Cédula: </span>
          <span>{{ this.factura.rncMedico }}</span>
        </h5> -->
        <h5>
      ...
      <div class="caja c2">
        ...
        <h5>
          ...
          <span>{{ formatSecuence(factura.no) }}</span>
        </h5>
        <!-- <h5>
        ...
        <h5>
          ...
          <span>{{ formatDate(this.factura.fechaProceso) }}</span>
        </h5>
        <!-- <h5>
          <span class="bold">Vencimiento: </span>
          <span>{{ formatDate(this.factura.vencimiento) }}</span>
        </h5> -->
      </div>
      ...
      <div class="caja c3">
        <!-- <h5>
          ...
          <span>{{ this.factura.rncCliente }}</span>
        </h5> -->
        <h5>
          <span class="bold">Cliente: </span>
          <span>{{ this.factura.descCliente }}</span>
        </h5>
      </div>
      <div class="caja c4">
        <table class="table table-hover">
          ...
          <tr>
            <td class="t izq">{{ this.factura.descServicios }}</td>
            <td class="t der">
              {{ formatNumber(this.factura.prefactura) }}
            </td>
          </tr>
        </table>
      </div>
      ...
      <div class="caja c6">
        <h6>{{ formatNumber(this.factura.prefactura) }}</h6>
        <h6>{{ formatNumber(this.factura.descuento) }}</h6>
        <h6>{{ formatNumber(this.factura.descuento) }}</h6>
        <h6 class="bold">
          {{ formatNumber(this.factura.prefactura) }}
        </h6>
  ...
  methods: {
    ...
    async loadFactura(id: string) {
      ...
      if (this.$store.state.ids.length == 0) {
        this.$router.push(`/facturas/${this.factura._id}`);
      } else {
        this.$router.push(`/facturaslot/new`);
      }
