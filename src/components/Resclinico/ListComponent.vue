<template>
  <table v-if="this.totales.resclinicos" id="customers">
    <tr>
      <th>Fecha / Hora</th>
      <th>Paciente</th>
    ...
    <tr
      v-for="(resclinico, index) in resclinicos"
      :key="index"
      @click="this.$router.push(`/resclinicos/${resclinico.id}`)"
    >
      <td>{{ formatDate(resclinico.fechaActual) }}</td>
      <td>{{ resclinico.paciente }}</td>
      <td>{{ resclinico.identificacion }}</td>
    </tr>
    <tr>
      <td>Total: {{ formatNumber(this.totales.resclinicos) }}</td>
      <td></td>
      <td class="der"></td>
    </tr>
  </table>

  <h6 class="noRegist" v-if="!this.totales.resclinicos">
...
import { updateOne } from "@/services/consultasdrmorales/Resclinico";
...
export default {
  ...
  methods: {
    ...
    async marcarListo(resclinico: Resclinico) {
      alert("Resclinico Lista.");
      try {
        const res = await updateOne(resclinico);
      } catch (error) {
        // console.error(error);
      }
    },
    ...
    toColor2(type: string) {
      ...
      } else if (type == "Resclinico") {
        return "table-warning";
      }
    },
    valorTotal() {
      this.totales.resclinicos = this.resclinicos.length;
      this.totales.valor = this.resclinicos.reduce(
