<template>
  <table v-if="this.totales.funcions" id="customers">
    <tr>
      <th>Descripción</th>
    </tr>
    <tr
      v-for="(funcion, index) in funcions"
      :key="index"
      @click="this.$router.push(`/funcions/${funcion._id}`)"
    >
      <td>
        {{ funcion.descripcion }}
      </td>
    ...
    <tr>
      <td>Total: {{ formatNumber(this.totales.funcions) }}</td>
    </tr>
  </table>

  <h6 class="noRegist" v-if="!this.totales.funcions">
...
import { updateOne } from "@/services/expedientesclinicos/Funcion";
...
export default {
  ...
  methods: {
    ...
    async marcarListo(funcion: Funcion) {
      alert("Funcion Lista.");
      try {
        const res = await updateOne(funcion);
      } catch (error) {
        // console.error(error);
      }
    },
    ...
    valorTotal() {
      this.totales.funcions = this.funcions.length;
      this.totales.cobertura = this.funcions.reduce(
