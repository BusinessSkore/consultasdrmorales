<template>
  <table v-if="this.totales.recetas" id="customers">
    <tr>
      <th>Fecha / Hora</th>
      <th>Paciente</th>
      <th>Edad</th>
    ...
    >
      <td>{{ formatDate(receta.fechaActual) }}</td>
      <td>{{ receta.seguro }}</td>
      <td>{{ receta.edad }} Años</td>
      
    </tr>
    <tr>
      <td>Total: {{ formatNumber(this.totales.recetas) }}</td>
      <td></td>
      <td class="der"></td>
    </tr>
  </table>

  <h6 class="noRegist" v-if="!this.totales.recetas">
...
import { updateOne } from "@/services/expedientesclinicos/Receta";
...
export default {
  ...
  methods: {
    ...
    async marcarListo(receta: Receta) {
      alert("Receta Lista.");
      try {
        const res = await updateOne(receta);
      } catch (error) {
        // console.error(error);
      }
    },
    ...
    valorTotal() {
      this.totales.recetas = this.recetas.length;
      this.totales.valor = this.recetas.reduce(
