<template>
  <table v-if="this.totales.licencias" id="customers">
    <tr>
      <th>Fecha / Hora</th>
      <th>Paciente</th>
    ...
    >
      <td>{{ formatDate(licencia.fechaActual) }}</td>
      <td>{{ licencia.nombre }}</td>
      <td>{{ licencia.seguro }}</td>
    </tr>
    <tr>
      <td>Total: {{ formatNumber(this.totales.licencias) }}</td>
      <td></td>
      <td class="der"></td>
    </tr>
  </table>

  <h6 class="noRegist" v-if="!this.totales.licencias">
...
import { updateOne } from "@/services/expedientesclinicos/Licencia";
...
export default {
  ...
  methods: {
    ...
    async marcarListo(licencia: Licencia) {
      alert("Licencia Lista.");
      try {
        const res = await updateOne(licencia);
      } catch (error) {
        // console.error(error);
      }
    },
    ...
    valorTotal() {
      this.totales.licencias = this.licencias.length;
      this.totales.valor = this.licencias.reduce(
