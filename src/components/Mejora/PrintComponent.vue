<template>
  ...
  <body v-show="!cargando">
    <div class="grid-layout">
      ...
      <div class="caja c1">
        <h4 class="bold">{{ this.mejora.medico }}</h4>
        <h5>{{ this.mejora.direccion }}</h5>
        <!-- <h5>
          <span class="bold">RNC / Cédula: </span>
          <span>{{ this.mejora.rncMedico }}</span>
        </h5> -->
        <h5>
      ...
      <div class="caja c2">
        ...
        <h5>
          ...
          <span>{{ formatSecuence(mejora.no) }}</span>
        </h5>
        <!-- <h5>
        ...
        <h5>
          ...
          <span>{{ formatDate(this.mejora.fechaProceso) }}</span>
        </h5>
        <!-- <h5>
          <span class="bold">Vencimiento: </span>
          <span>{{ formatDate(this.mejora.vencimiento) }}</span>
        </h5> -->
      </div>
      ...
      <div class="caja c3">
        <!-- <h5>
          ...
          <span>{{ this.mejora.rncCliente }}</span>
        </h5> -->
        <h5>
          <span class="bold">Cliente: </span>
          <span>{{ this.mejora.descCliente }}</span>
        </h5>
      </div>
      <div class="caja c4">
        <table class="table table-hover">
          ...
          <tr>
            <td class="t izq">{{ this.mejora.descServicios }}</td>
            <td class="t der">
              {{ formatNumber(this.mejora.prefactura) }}
            </td>
          </tr>
        </table>
      </div>
      ...
      <div class="caja c6">
        <h6>{{ formatNumber(this.mejora.prefactura) }}</h6>
        <h6>{{ formatNumber(this.mejora.descuento) }}</h6>
        <h6>{{ formatNumber(this.mejora.descuento) }}</h6>
        <h6 class="bold">
          {{ formatNumber(this.mejora.prefactura) }}
        </h6>
  ...
  methods: {
    ...
    async loadMejora(id: string) {
      ...
      if (this.$store.state.ids.length == 0) {
        this.$router.push(`/mejoras/${this.mejora._id}`);
      } else {
        this.$router.push(`/mejoraslot/new`);
      }
